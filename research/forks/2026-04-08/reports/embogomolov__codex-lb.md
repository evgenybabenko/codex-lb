# embogomolov/codex-lb

- URL: https://github.com/embogomolov/codex-lb
- Снимок: `2026-04-08`
- Default branch: `main`
- Создан форк: `2026-04-06T15:08:23Z`
- Последний push: `2026-04-07T09:21:53Z`
- Веток в форке: `23`
...
- Статус: behind, ahead 0, behind 18.

## Default branch vs наш develop

- Статус: diverged, ahead 24, behind 6.
...
- `sync/cross-repo-parity-codex`: ahead 9, behind 19. По коммитам видно: restore capacity-based usage donut totals; cache-invalidation; bootstrap-token-security.
  - Коммиты: restore capacity-based usage donut totals; cache-invalidation; bootstrap-token-security; fail-closed-proxy-auth.
  - Основные зоны: `frontend/src` (7), `frontend/dashboard` (3), `app/modules/dashboard_auth` (2), `app/core/auth` (1).
...
- `feat/model-context-window-overrides`: ahead 3, behind 18. По коммитам видно: add model_context_window_overrides setting; address CI failures in type check and test; reject bool/float in context window overrides, run ruff format.
  - Коммиты: add model_context_window_overrides setting; address CI failures in type check and test; reject bool/float in context window overrides, run ruff format.
  - Основные зоны: `app/core/config` (1), `app/modules/proxy` (1), `tests/integration` (1).
