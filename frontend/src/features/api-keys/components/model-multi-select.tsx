...
export function ModelMultiSelect({
  ...
  const label =
    value.length === 0
      ...
      : t("apiModelMultiSelected", { count: value.length });
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            disabled={isLoading}
          >
            <span className="truncate text-left">{isLoading ? t("commonLoadingModels") : label}</span>
        ...
        <DropdownMenuContent align="start" className="w-[var(--radix-dropdown-menu-trigger-width)] max-h-64">
          ...
          {filtered.length === 0 ? (
            <div className="px-2 py-1.5 text-xs text-muted-foreground">{t("commonNoModelsFound")}</div>
          ) : null}
        </DropdownMenuContent>
      </DropdownMenu>
