...
export function WindowsOauthHelp() {
  ...
  return (
    <div className="space-y-3 rounded-xl border bg-card p-4">
      <div>
        <h2 className="text-sm font-semibold">{t("accountWindowsHelpTitle")}</h2>
        <p className="mt-1 text-xs text-muted-foreground">
          {t("accountWindowsHelpDescription")}{" "}
          <code className="mx-1 rounded bg-muted px-1 py-0.5">localhost:1455</code>
        </p>
      </div>

      <p className="text-xs text-muted-foreground">
        {t("accountWindowsHelpRuntimeAddress")}:
      ...
      <div className="flex flex-wrap items-center gap-2">
        <CopyButton value={commands} label={t("accountWindowsHelpCopyCommands")} />
        <span className="text-xs text-muted-foreground">{t("accountWindowsHelpRunAsAdmin")}</span>
      </div>
    </div>
  );
}
